...
function getKeysFromHtml(filePath) {
  ...
  const content = fs.readFileSync(filePath, "utf-8");
  const keys = new Set();
  // Match data-i18n, data-i18n-html, data-i18n-placeholder, data-i18n-aria-label, data-i18n-title, data-i18n-alt
  const matches = content.matchAll(
    /data-i18n(?:-html|-placeholder|-aria-label|-title|-alt)?=["']([a-zA-Z0-9_-]+)["']/g,
  );
  for (const match of matches) {
    keys.add(match[1]);
  }
